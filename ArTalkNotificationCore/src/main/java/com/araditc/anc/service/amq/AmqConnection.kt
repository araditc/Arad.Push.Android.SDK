 ...
 */
internal class AmqConnection(
    private val service: AmqService, // fields for the connection definition
    ...
    fun connect(options: MqttConnectOptions?, invocationContext: String?, activityToken: String?) {
        ...
        options?.let {
            ...
            if (it.isCleanSession) { // if it's a clean session,
                // discard old data
                if (service.messageDatabase.isOpen) CoroutineScope(Dispatchers.IO).launch {
                    service.messageDatabase.persistenceDao().deleteClientHandle(clientHandle)
                }
                else Log.i(TAG, "Database is closed")
            }
        }
        service.traceDebug("Connecting {$serverURI} as {$clientId}")
        val resultBundle = Bundle()
        resultBundle.putString(AmqServiceConstants.CALLBACK_ACTIVITY_TOKEN, activityToken)
        resultBundle.putString(AmqServiceConstants.CALLBACK_INVOCATION_CONTEXT, invocationContext)
        resultBundle.putString(
            AmqServiceConstants.CALLBACK_ACTION, AmqServiceConstants.CONNECT_ACTION
        )
        try {
            if (persistence == null) {
            ...
            val listener: IMqttActionListener = object : MqttConnectionListener(resultBundle) {
                override fun onSuccess(asyncActionToken: IMqttToken) {
                    resultBundle.putBoolean(
                        AmqServiceConstants.SESSION_PRESENT, asyncActionToken.sessionPresent
                    )
                ...
                override fun onFailure(asyncActionToken: IMqttToken?, exception: Throwable?) {
                    resultBundle.putString(
                        AmqServiceConstants.CALLBACK_ERROR_MESSAGE, exception?.localizedMessage
                    )
                    resultBundle.putSerializable(AmqServiceConstants.CALLBACK_EXCEPTION, exception)
    ...
    override fun connectComplete(reconnect: Boolean, serverURI: String) {
        val resultBundle = Bundle()
        resultBundle.putString(
            AmqServiceConstants.CALLBACK_ACTION, AmqServiceConstants.CONNECT_EXTENDED_ACTION
        )
        resultBundle.putBoolean(AmqServiceConstants.CALLBACK_RECONNECT, reconnect)
        resultBundle.putString(AmqServiceConstants.CALLBACK_SERVER_URI, serverURI)
        service.callbackToActivity(clientHandle, Status.OK, resultBundle)
    }
    ...
    private fun handleException(resultBundle: Bundle, e: Exception) {
        resultBundle.putString(AmqServiceConstants.CALLBACK_ERROR_MESSAGE, e.localizedMessage)
        resultBundle.putSerializable(AmqServiceConstants.CALLBACK_EXCEPTION, e)
        service.callbackToActivity(clientHandle, Status.ERROR, resultBundle)
    }
    ...
    private fun deliverBacklog() {
        CoroutineScope(Dispatchers.IO).launch {
            service.messageDatabase.persistenceDao().allArrived(clientHandle).forEach {
                ...
                resultBundle.putString(
                    AmqServiceConstants.CALLBACK_ACTION,
                    AmqServiceConstants.MESSAGE_ARRIVED_ACTION
                )
                service.callbackToActivity(clientHandle, Status.OK, resultBundle)
            }
        }
    ...
    private fun messageToBundle(messageId: String?, topic: String?, message: MqttMessage): Bundle {
        val result = Bundle()
        result.putString(AmqServiceConstants.CALLBACK_MESSAGE_ID, messageId)
        result.putString(AmqServiceConstants.CALLBACK_DESTINATION_NAME, topic)
        result.putParcelable(
            AmqServiceConstants.CALLBACK_MESSAGE_PARCEL, ParcelableMqttMessage(message)
        )
        return result
    }
    ...
    fun disconnect(quiesceTimeout: Long, invocationContext: String?, activityToken: String?) {
        ...
        resultBundle.putString(AmqServiceConstants.CALLBACK_INVOCATION_CONTEXT, invocationContext)
        resultBundle.putString(
            AmqServiceConstants.CALLBACK_ACTION, AmqServiceConstants.DISCONNECT_ACTION
        )
        if (myClient != null && myClient!!.isConnected) {
            ...
            resultBundle.putString(AmqServiceConstants.CALLBACK_ERROR_MESSAGE, NOT_CONNECTED)
            service.traceError(AmqServiceConstants.DISCONNECT_ACTION + " " + NOT_CONNECTED)
            service.callbackToActivity(clientHandle, Status.ERROR, resultBundle)
        }
    ...
    fun publish(
        ...
        val resultBundle = Bundle()
        resultBundle.putString(
            AmqServiceConstants.CALLBACK_ACTION, AmqServiceConstants.SEND_ACTION
        )
        resultBundle.putString(AmqServiceConstants.CALLBACK_ACTIVITY_TOKEN, activityToken)
        resultBundle.putString(AmqServiceConstants.CALLBACK_INVOCATION_CONTEXT, invocationContext)
        ...
        if (myClient != null && myClient!!.isConnected) {
            ...
            try {
                ...
                sendToken = myClient!!.publish(
                    topic, payload, qos.value, retained, invocationContext, listener
                )
            ...
            resultBundle.putString(AmqServiceConstants.CALLBACK_ERROR_MESSAGE, NOT_CONNECTED)
            service.traceError(AmqServiceConstants.SEND_ACTION + " " + NOT_CONNECTED)
            service.callbackToActivity(clientHandle, Status.ERROR, resultBundle)
        }
        return sendToken
    }
     ...
     */
    fun publish(
        topic: String, message: MqttMessage, invocationContext: String?, activityToken: String
    ): IMqttDeliveryToken? {
        val resultBundle = Bundle()
        resultBundle.putString(
            AmqServiceConstants.CALLBACK_ACTION, AmqServiceConstants.SEND_ACTION
    ...
    fun subscribe(topic: String, qos: QoS, invocationContext: String?, activityToken: String) {
        ...
        val resultBundle = Bundle()
        resultBundle.putString(
            AmqServiceConstants.CALLBACK_ACTION, AmqServiceConstants.SUBSCRIBE_ACTION
        )
        resultBundle.putString(AmqServiceConstants.CALLBACK_ACTIVITY_TOKEN, activityToken)
        resultBundle.putString(AmqServiceConstants.CALLBACK_INVOCATION_CONTEXT, invocationContext)
        if (myClient != null && myClient!!.isConnected) {
     ...
     */
    fun subscribe(
        topic: Array<String>, qos: IntArray?, invocationContext: String?, activityToken: String
    ) {
        service.traceDebug(
            "subscribe({" + topic.contentToString() + "}," + Arrays.toString(qos) + ",{" + invocationContext + "}, {" + activityToken + "}"
        )
        val resultBundle = Bundle()
        resultBundle.putString(
            AmqServiceConstants.CALLBACK_ACTION, AmqServiceConstants.SUBSCRIBE_ACTION
    ...
    fun subscribe(
        ...
        service.traceDebug(
            "subscribe({" + topicFilters.contentToString() + "}," + qos.contentToString() + ",{" + invocationContext + "}, {" + activityToken + "}"
        )
        val resultBundle = Bundle()
        resultBundle.putString(
            AmqServiceConstants.CALLBACK_ACTION, AmqServiceConstants.SUBSCRIBE_ACTION
    ...
    fun unsubscribe(topic: String, invocationContext: String?, activityToken: String) {
        ...
        val resultBundle = Bundle()
        resultBundle.putString(
            AmqServiceConstants.CALLBACK_ACTION, AmqServiceConstants.UNSUBSCRIBE_ACTION
        )
        resultBundle.putString(AmqServiceConstants.CALLBACK_ACTIVITY_TOKEN, activityToken)
        resultBundle.putString(AmqServiceConstants.CALLBACK_INVOCATION_CONTEXT, invocationContext)
        if (myClient != null && myClient!!.isConnected) {
    ...
    override fun connectionLost(why: Throwable?) {
        ...
        val resultBundle = Bundle()
        resultBundle.putString(
            AmqServiceConstants.CALLBACK_ACTION, AmqServiceConstants.ON_CONNECTION_LOST_ACTION
        )
        if (why != null) {
            resultBundle.putString(AmqServiceConstants.CALLBACK_ERROR_MESSAGE, why.message)
            if (why is MqttException) {
                resultBundle.putSerializable(AmqServiceConstants.CALLBACK_EXCEPTION, why)
            }
            resultBundle.putString(
                AmqServiceConstants.CALLBACK_EXCEPTION_STACK, Log.getStackTraceString(why)
            )
        }
        service.callbackToActivity(clientHandle, Status.OK, resultBundle)
    ...
    override fun deliveryComplete(messageToken: IMqttDeliveryToken) {
        ...
        if (resultBundle != null) {
            if (AmqServiceConstants.SEND_ACTION == resultBundle.getString(AmqServiceConstants.CALLBACK_ACTION)) {
                service.callbackToActivity(clientHandle, Status.OK, resultBundle)
            }
            resultBundle.putString(
                AmqServiceConstants.CALLBACK_ACTION, AmqServiceConstants.MESSAGE_DELIVERED_ACTION
            )
            service.callbackToActivity(clientHandle, Status.OK, resultBundle)
        }
    ...
    override fun messageArrived(topic: String, message: MqttMessage) {
        ...
        resultBundle.putString(
            AmqServiceConstants.CALLBACK_ACTION, AmqServiceConstants.MESSAGE_ARRIVED_ACTION
        )
        resultBundle.putString(AmqServiceConstants.CALLBACK_MESSAGE_ID, messageId)
        service.callbackToActivity(clientHandle, Status.OK, resultBundle)
    }
    ...
    private fun popSendDetails(messageToken: IMqttDeliveryToken): Bundle? {
        ...
        if (message != null) { // If I don't know about the message, it's
            ...
            if (activityToken != null) {
                ...
                resultBundle.putString(AmqServiceConstants.CALLBACK_ACTIVITY_TOKEN, activityToken)
                resultBundle.putString(
                    AmqServiceConstants.CALLBACK_INVOCATION_CONTEXT, invocationContext
                )
            }
            return resultBundle
        }
    ...
    fun reconnect(context: Context) {
        ...
        if (connectOptions!!.isAutomaticReconnect) {
            ...
            val resultBundle = Bundle()
            resultBundle.putString(
                AmqServiceConstants.CALLBACK_ACTIVITY_TOKEN, reconnectActivityToken
            )
            resultBundle.putString(AmqServiceConstants.CALLBACK_INVOCATION_CONTEXT, null)
            resultBundle.putString(
                AmqServiceConstants.CALLBACK_ACTION, AmqServiceConstants.CONNECT_ACTION
            )
            CoroutineScope(Dispatchers.IO).launch {
                try {
                    ...
                    CoroutineScope(Dispatchers.Main).launch {
                        Log.i(TAG, "Exception occurred attempting to reconnect: ${ex.message}")
                        setConnectingState(false)
            ...
            try {
                val listener: IMqttActionListener = object : MqttConnectionListener(resultBundle) {
                    ...
                    override fun onFailure(asyncActionToken: IMqttToken?, exception: Throwable?) {
                        ...
                        resultBundle.putSerializable(
                            AmqServiceConstants.CALLBACK_EXCEPTION, exception
                        )
                        service.callbackToActivity(clientHandle, Status.ERROR, resultBundle)
                        doAfterConnectFail(resultBundle)
                    }
                ...
                setConnectingState(false)
                handleException(resultBundle, e)
            } catch (e: Exception) {/*  TODO: Added Due to: https://github.com/eclipse/paho.mqtt.android/issues/101
