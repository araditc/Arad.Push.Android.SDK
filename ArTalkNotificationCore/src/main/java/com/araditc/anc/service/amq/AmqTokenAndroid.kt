...
internal open class AmqTokenAndroid constructor(
    private val client: AmqAndroidClient,
    ...
    override fun waitForCompletion(timeout: Long) {
        ...
        if (!isComplete) {
            throw MqttException(
                MqttException.REASON_CODE_CLIENT_TIMEOUT.toInt(), Throwable("After $timeout ms")
            )
        }
        pendingException?.let { throw it }
    }
