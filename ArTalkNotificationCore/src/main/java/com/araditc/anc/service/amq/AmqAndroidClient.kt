...
import com.araditc.anc.service.amq.extension.parcelable
import com.araditc.anc.service.amq.extension.serializable
 ...
 *
 */
class AmqAndroidClient @JvmOverloads constructor(
    val context: Context,
    ...
    private var mqttService: AmqService? = null
    ...
    private var traceCallback: AmqTraceHandler? = null
    ...
    override fun isConnected() =
        clientHandle != null && mqttService != null && mqttService!!.isConnected(clientHandle!!)
    ...
    override fun close() {
        mqttService?.let {
            if (clientHandle == null) {
                clientHandle = it.getClient(
                    serverURI, clientId, context.applicationInfo.packageName, persistence
                )
            }
    ...
    override fun connect(
        options: MqttConnectOptions, userContext: Any?, callback: IMqttActionListener?
    ): IMqttToken {
        val token: IMqttToken = AmqTokenAndroid(this, userContext, callback)
        ...
        if (mqttService == null) { // First time - must bind to the service
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && foregroundServiceNotification != null) {
                serviceStartIntent.putExtra(
                    AmqService.MQTT_FOREGROUND_SERVICE_NOTIFICATION, foregroundServiceNotification
                )
                serviceStartIntent.putExtra(
                    AmqService.MQTT_FOREGROUND_SERVICE_NOTIFICATION_ID, FOREGROUND_ID
                )
    ...
    override fun disconnect(): IMqttToken {
        val token: IMqttToken = AmqTokenAndroid(this, null, null)
    ...
    override fun disconnect(
        quiesceTimeout: Long, userContext: Any?, callback: IMqttActionListener
    ): IMqttToken {
        val token: IMqttToken = AmqTokenAndroid(this, userContext, callback)
        val activityToken = storeToken(token)
        mqttService!!.disconnect(clientHandle!!, quiesceTimeout, null, activityToken)
    ...
    override fun publish(
        ...
        val token = AmqDeliveryTokenAndroid(this, userContext, callback, message)
        val activityToken = storeToken(token)
        val internalToken = mqttService!!.publish(
            clientHandle!!, topic, payload, QoS.valueOf(qos), retained, null, activityToken
        )
     ...
     */
    override fun publish(
        topic: String, message: MqttMessage, userContext: Any?, callback: IMqttActionListener?
    ): IMqttDeliveryToken {
        val token = AmqDeliveryTokenAndroid(this, userContext, callback, message)
        val activityToken = storeToken(token)
        val internalToken =
    ...
    override fun subscribe(
        topic: String, qos: Int, userContext: Any?, callback: IMqttActionListener?
    ): IMqttToken {
        val token: IMqttToken = AmqTokenAndroid(this, userContext, callback, arrayOf(topic))
        val activityToken = storeToken(token)
     ...
     */
    override fun subscribe(
        topic: Array<String>, qos: IntArray, userContext: Any?, callback: IMqttActionListener?
    ): IMqttToken {
        val token: IMqttToken = AmqTokenAndroid(this, userContext, callback, topic)
        val activityToken = storeToken(token)
    ...
    override fun subscribe(
        ...
        return subscribe(
            arrayOf(topicFilter), intArrayOf(qos), userContext, callback, arrayOf(messageListener)
        )
    }
     ...
     */
    override fun subscribe(
        topicFilter: String, qos: Int, messageListener: IMqttMessageListener
    ): IMqttToken {
     ...
     */
    override fun subscribe(
        topicFilters: Array<String>, qos: IntArray, messageListeners: Array<IMqttMessageListener>
    ): IMqttToken {
    ...
    override fun subscribe(
        ...
        val token: IMqttToken = AmqTokenAndroid(this, userContext, callback, topicFilters)
        val activityToken = storeToken(token)
        mqttService!!.subscribe(
            clientHandle!!,
            topicFilters,
            qos.map { QoS.valueOf(it) }.toTypedArray(),
            null,
            activityToken,
            messageListeners
        )
    ...
    override fun unsubscribe(
        topic: String, userContext: Any?, callback: IMqttActionListener?
    ): IMqttToken {
        val token: IMqttToken = AmqTokenAndroid(this, userContext, callback)
        val activityToken = storeToken(token)
     ...
     */
    override fun unsubscribe(
        topic: Array<String>, userContext: Any?, callback: IMqttActionListener?
    ): IMqttToken {
        val token: IMqttToken = AmqTokenAndroid(this, userContext, callback)
        val activityToken = storeToken(token)
        mqttService!!.unsubscribe(clientHandle!!, topic, null, activityToken)
     ...
     */
    fun setTraceCallback(traceCallback: AmqTraceHandler?) {
    ...
    private fun onReceive(data: Bundle) {
        ...
        val handleFromIntent = data.getString(AmqServiceConstants.CALLBACK_CLIENT_HANDLE)
        if (handleFromIntent == null || handleFromIntent != clientHandle) {
            return
        }
        val action = data.getString(AmqServiceConstants.CALLBACK_ACTION)
        if (AmqServiceConstants.CONNECT_ACTION == action) {
            connectAction(data)
        } else if (AmqServiceConstants.CONNECT_EXTENDED_ACTION == action) {
            connectExtendedAction(data)
        } else if (AmqServiceConstants.MESSAGE_ARRIVED_ACTION == action) {
            messageArrivedAction(data)
        } else if (AmqServiceConstants.SUBSCRIBE_ACTION == action) {
            subscribeAction(data)
        } else if (AmqServiceConstants.UNSUBSCRIBE_ACTION == action) {
            unSubscribeAction(data)
        } else if (AmqServiceConstants.SEND_ACTION == action) {
            sendAction(data)
        } else if (AmqServiceConstants.MESSAGE_DELIVERED_ACTION == action) {
            messageDeliveredAction(data)
        } else if (AmqServiceConstants.ON_CONNECTION_LOST_ACTION == action) {
            connectionLostAction(data)
        } else if (AmqServiceConstants.DISCONNECT_ACTION == action) {
            disconnected(data)
        } else if (AmqServiceConstants.TRACE_ACTION == action) {
            traceAction(data)
        } else {
    ...
    private fun connectAction(data: Bundle?) {
        ...
        (token as AmqTokenAndroid?)!!.setDelegate(
            AmqConnectTokenAndroid(
                data!!.getBoolean(
                    AmqServiceConstants.SESSION_PRESENT
                )
            )
        )
    ...
    private fun disconnected(data: Bundle?) {
        ...
        token?.let {
            (it as AmqTokenAndroid).notifyComplete()
        }
    ...
    private fun connectionLostAction(data: Bundle?) {
        val reason = data!!.parcelable(AmqServiceConstants.CALLBACK_EXCEPTION) as Exception?
        callbacksList.forEach {
    ...
    private fun connectExtendedAction(data: Bundle?) {
        ...
        val reconnect = data!!.getBoolean(AmqServiceConstants.CALLBACK_RECONNECT, false)
        val serverURI = data.getString(AmqServiceConstants.CALLBACK_SERVER_URI)
    ...
    private fun simpleAction(token: IMqttToken?, data: Bundle) {
        if (token != null) {
            val status = data.serializable(AmqServiceConstants.CALLBACK_STATUS) as Status?
            if (status == Status.OK) {
                (token as AmqTokenAndroid).notifyComplete()
            } else {
                val errorMessage =
                    data.serializable(AmqServiceConstants.CALLBACK_ERROR_MESSAGE) as String?
                var exceptionThrown =
                    data.serializable(AmqServiceConstants.CALLBACK_EXCEPTION) as Throwable?
                if (exceptionThrown == null && errorMessage != null) {
                    exceptionThrown = Throwable(errorMessage)
                } else if (exceptionThrown == null) {
                    val bundleToString = data.keySet().joinToString(", ", "{", "}") { key ->
                        "$key=${data.getString(key)}"
                    }
                    exceptionThrown = Throwable("No Throwable given\n$bundleToString")
                }
                (token as AmqTokenAndroid).notifyFailure(exceptionThrown)
            }
        } else {
    ...
    private fun messageArrivedAction(data: Bundle?) {
        val messageId = data!!.getString(AmqServiceConstants.CALLBACK_MESSAGE_ID)!!
        val destinationName = data.getString(AmqServiceConstants.CALLBACK_DESTINATION_NAME)
        val message: ParcelableMqttMessage =
            data.parcelable(AmqServiceConstants.CALLBACK_MESSAGE_PARCEL)!!
        try {
    ...
    private fun traceAction(data: Bundle?) {
        traceCallback?.let {
            val severity = data!!.getString(AmqServiceConstants.CALLBACK_TRACE_SEVERITY)
            val message = data.getString(AmqServiceConstants.CALLBACK_ERROR_MESSAGE)
            when (severity) {
                AmqServiceConstants.TRACE_DEBUG -> it.traceDebug(message)
                AmqServiceConstants.TRACE_ERROR -> it.traceError(message)
                else -> {
                    val e = data.serializable(AmqServiceConstants.CALLBACK_EXCEPTION) as Exception?
    ...
    private fun removeMqttToken(data: Bundle?): IMqttToken? {
        val activityToken = data!!.getString(AmqServiceConstants.CALLBACK_ACTIVITY_TOKEN)
    ...
    fun unregisterResources() {
        if (receiverRegistered.get()) {
            synchronized(this@AmqAndroidClient) {
                clientJob?.cancel()
                clientJob = null
                clientScope = null
    ...
    private inner class MyServiceConnection : ServiceConnection {
        override fun onServiceConnected(name: ComponentName, binder: IBinder) {
            if (AmqServiceBinder::class.java.isAssignableFrom(binder.javaClass)) {
                mqttService = (binder as AmqServiceBinder).service
                serviceBound = true
                collect()
    ...
    companion object {
        private val SERVICE_NAME = AmqService::class.java.name
