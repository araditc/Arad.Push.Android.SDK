...
class MqttService : Service(), MqttTraceHandler {
        ...
        traceCallbackId?.let {
            if (isTraceEnabled) {
                ...
                dataBundle.putString(
                    MqttServiceConstants.CALLBACK_ACTION, MqttServiceConstants.TRACE_ACTION
                )
    ...
    override fun traceException(message: String?, e: Exception?) {
        traceCallbackId?.let {
            ...
            dataBundle.putString(
                MqttServiceConstants.CALLBACK_TRACE_SEVERITY, MqttServiceConstants.TRACE_EXCEPTION
            )
            dataBundle.putString(MqttServiceConstants.CALLBACK_ERROR_MESSAGE, message)
    ...
    companion object {
        val MQTT_FOREGROUND_SERVICE_NOTIFICATION_ID =
            MqttService::class.java.simpleName + ".FOREGROUND_SERVICE_NOTIFICATION_ID"
        val MQTT_FOREGROUND_SERVICE_NOTIFICATION =
            MqttService::class.java.simpleName + ".FOREGROUND_SERVICE_NOTIFICATION"
    }
}
